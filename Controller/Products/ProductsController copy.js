...
const UpdateProductOperation = async (req, res) => {
  ...
  const { title, price, description, category, productId, Brand } = req.body;
  let product =
  {
    title: title,
    price: price,
  ...
  try {
    ...
    if (errors.length > 0) {
      ...
      console.log("Previous Data:", prevouseData);
      ...
      const rs = await ProductModel.findByIdAndUpdate(productId, product);
      if (rs) {
